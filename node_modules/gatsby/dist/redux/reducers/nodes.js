...
var findChildrenRecursively = function findChildrenRecursively() {
  ...
  children = (_children = children).concat.apply(_children, children.map(function (child) {
    var newChildren = parentChildrenMap.get(child);
    ...
    case `CREATE_NODE`:
      {
        ...
        // to be recreated.
        var descendantNodes = findChildrenRecursively(parentChildrenMap.get(action.payload.id));
    ...
    case `DELETE_NODE`:
      {
        ...
        var allNodes = [action.payload].concat(findChildrenRecursively(parentChildrenMap.get(action.payload)));
    ...
    case `DELETE_NODES`:
      {
        ...
        var _allNodes = [].concat(action.payload, _.flatten(action.payload.map(function (n) {
          var descendantNodes = findChildrenRecursively(parentChildrenMap.get(n));
          parentChildrenMap.delete(n);
          return descendantNodes;
        })));
        newState = _.omit(state, _allNodes);
        return newState;
      }
